    ...
    async def process_batch(self, prompts: List[Dict[str, Any]]) -> List[EnhancedPromptResult]:
        """Enhanced batch processing with progress tracking for multiple services"""
        
        if not await self.discover_working_endpoint():
            raise ConnectionError("No working endpoint found")
        
        logger.info(f"Processing {len(prompts)} prompts using: {self.detected_service.upper()}")
        if self.working_endpoint:
            logger.info(f"Endpoint: {self.working_endpoint}")
        
        self.start_time = time.time()
        
        # For OpenAI, we don't need aiohttp session
        if self.detected_service == "openai":
            results = []
            for i, prompt_data in enumerate(prompts):
                try:
                    result = await self._process_single_prompt(None, prompt_data)
                    ...
                    if (i + 1) % 10 == 0:
                        ...
                        eta = (len(prompts) - processed) / rate if rate > 0 else 0
                        
                        logger.info(f"Progress: {processed}/{len(prompts)} ({processed/len(prompts)*100:.1f}%) | "
    ...
    def save_comprehensive_results(self, output_file: str = None, input_filename: str = None) -> Dict[str, str]:
        ...
        if input_filename:
            ...
            base_name = f"{self.detected_service}_{input_base}_{timestamp}"
        else:
            base_name = f"{self.detected_service}_content_safety_results_{timestamp}"
        
        output_files = {}
        
        # 1. Main results CSV
        results_csv_file = self.output_directory / f"{base_name}_results.csv"
        self._save_results_csv(results_csv_file)
        output_files['results_csv'] = str(results_csv_file)
        
        # 2. Main results Parquet (if PyArrow available)
        if PYARROW_AVAILABLE:
            try:
                results_parquet_file = self.output_directory / f"{base_name}_results.parquet"
                self._save_results_parquet(results_parquet_file)
                output_files['results_parquet'] = str(results_parquet_file)
            except Exception as e:
                logger.warning(f"Could not save Parquet results: {str(e)}")
        ...
        for file_type, file_path in output_files.items():
            ...
            logger.info(f"  {file_type.title()}: {file_path} ({file_size:.2f} MB)")
    ...
    def _save_results_parquet(self, output_path: Path):
        """Save enhanced results as Parquet file"""
    ...
    def _optimize_dataframe_for_parquet(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bool_columns = ['ground_truth_binary']
        for col in bool_columns:
            if col in df_optimized.columns:
                df_optimized[col] = df_optimized[col].astype('boolean')
        
        # Convert categorical columns
        categorical_columns = ['decision', 'original_type', 'service_type']
        ...
        for col, dtype in numeric_optimizations.items():
            if col in df_optimized.columns:
                try:
                    ...
                    logger.debug(f"Could not optimize {col} to {dtype}: {str(e)}")
        ...
        if 'timestamp' in df_optimized.columns:
            try:
                ...
                logger.debug(f"Could not convert timestamp to datetime: {str(e)}")
        ...
        try:
            ...
            if 'error' not in metrics:
                ...
                metrics_data = [
                    ...
                    ['Service Type', self.detected_service or 'Unknown', 'Content safety service used', 'Service'],
                    ['API Endpoint', self.working_endpoint or 'Unknown', 'API endpoint used', 'Service'],
                    ['API Version', self.api_version or 'Unknown', 'API version', 'Service'],
                    ['Endpoint Type', getattr(self, 'endpoint_type', 'Unknown'), 'Azure endpoint type', 'Service'],
                    ['Severity Threshold', self.severity_threshold, 'Blocking threshold (0-6)', 'Service'],
                    ['Concurrent Requests', self.max_concurrent_requests, 'Max parallel requests', 'Service'],
                    ['Rate Limit', self.rate_limit_per_minute, 'Requests per minute limit', 'Service'],
    ...
    def _save_dataset_analysis_json(self, analysis_path: Path):
        ...
        try:
            analysis_data = {
                ...
                'service_config': {
                    'service_type': self.detected_service,
                    'endpoint': self.working_endpoint,
                    'api_version': self.api_version,
                    'endpoint_type': getattr(self, 'endpoint_type', 'Unknown'),
    ...
    def _save_summary_report(self, summary_path: Path, input_filename: str = None):
        ...
        try:
            ...
            with open(summary_path, 'w', encoding='utf-8') as f:
                f.write("="*80 + "\n")
                f.write("MULTI-CLOUD CONTENT SAFETY - COMPREHENSIVE ANALYSIS REPORT\n")
                f.write("="*80 + "\n\n")
                ...
                f.write("-" * 50 + "\n")
                f.write(f"Service Type: {(self.detected_service or 'Unknown').upper()}\n")
                f.write(f"Endpoint: {self.working_endpoint or 'Unknown'}\n")
                f.write(f"API Version: {self.api_version or 'Unknown'}\n")
                if hasattr(self, 'endpoint_type') and self.endpoint_type:
                    f.write(f"Endpoint Type: {self.endpoint_type.upper()}\n")
                ...
                f.write("\n" + "="*80 + "\n")
                f.write("Report generated by Multi-Cloud Content Safety Processor v5.1\n")
                f.write("="*80 + "\n")
    ...
    def _print_key_metrics(self, metrics: Dict[str, Any]):
        ...
        logger.info("="*60)
        logger.info(f"Service: {(self.detected_service or 'Unknown').upper()}")
    ...
    def _handle_list_columns(self, input_file: str):
        """Handle --list-columns option for CSV and Parquet files"""
        try:
            ...
            for i, col in enumerate(df.columns, 1):
                ...
                print(f"  {i:2d}. {col:<30} ({dtype}) - {null_count} nulls")
    ...
    def _print_final_summary(self, total_results: int, output_files: Dict[str, str]):
        ...
        print("="*80)
        print(f"Service Used: {(self.detected_service or 'Unknown').upper()}")
...
def check_dependencies():
    ...
    dependencies = {
        ...
        'scikit-learn': ('sklearn', SKLEARN_AVAILABLE),
        'numpy': ('numpy', NUMPY_AVAILABLE),
        'pyarrow': ('pyarrow', PYARROW_AVAILABLE),
        'matplotlib': ('matplotlib', PLOTTING_AVAILABLE),
        'openai': ('openai', OPENAI_AVAILABLE),
    }
    
    all_available = True
    
    for name, (import_name, available) in dependencies.items():
        ...
        if available:
            try:
                ...
                print(f"{'':17}  Version: {version}")
            except:
                pass
        else:
            if name in ['scikit-learn', 'numpy']:
                all_available = False
                print(f"{'':17}  ⚠️  Required for core functionality")
            elif name == 'pyarrow':
                print(f"{'':17}  💡 Install for Parquet support: pip install pyarrow")
            elif name == 'matplotlib':
                print(f"{'':17}  💡 Install for plot generation: pip install matplotlib seaborn")
            elif name == 'openai':
                print(f"{'':17}  💡 Install for OpenAI support: pip install openai")
    
    print("\nSUPPORTED SERVICES:")
    print(f"  Azure Content Safety: ✅ Always supported")
    print(f"  Azure Prompt Shield: ✅ Always supported") 
    print(f"  OpenAI Moderation: {'✅ Supported' if OPENAI_AVAILABLE else '❌ Requires openai package'}")
    
    print("\nSUPPORTED FILE FORMATS:")
    print(f"  CSV (.csv): ✅ Always supported")
    print(f"  Parquet (.parquet, .pq): {'✅ Supported' if PYARROW_AVAILABLE else '❌ Requires pyarrow'}")
    
    if not all_available:
        print("\n⚠️  Some required dependencies are missing. Install with:")
        print("   pip install pandas aiohttp scikit-learn numpy")
        print("\n💡 For full functionality, also install:")
        print("   pip install pyarrow matplotlib seaborn openai")
    else:
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Multi-Cloud Content Safety Processor v5.1 - Production Ready",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
🚀 PRODUCTION-READY MULTI-CLOUD CONTENT SAFETY PROCESSOR

Supported Services:
  • Azure Content Safety - Multi-category content analysis (Hate, Violence, etc.)
  • Azure Prompt Shield - Specialized jailbreak and prompt injection detection  
  • OpenAI Moderation - OpenAI's content moderation API

Enhanced Features:
  • Intelligent schema detection for multiple dataset types
  • Advanced ground truth mapping with confidence scores
  • Comprehensive performance metrics (binary classification focus)
  • Multiple output formats (CSV, Parquet, JSON, summary report)
  • Enhanced error handling and progress tracking
  • Business impact analysis and quality indicators
  • Support for both CSV and Parquet file formats

Supported File Formats:
  • CSV (.csv) - with multiple encoding and separator detection
  • Parquet (.parquet, .pq) - optimized columnar format with compression
...
Examples:
  ...
  python processor.py --input dataset.csv --api-key YOUR_KEY

  # Azure Content Safety (specific endpoint)
  python processor.py --input dataset.csv --endpoint AZURE_URL --api-key AZURE_KEY

  # OpenAI Moderation API
  python processor.py --input dataset.csv --service openai --api-key OPENAI_KEY

  # Parquet file processing
  python processor.py --input dataset.parquet --api-key YOUR_KEY

  # Analysis only (no API calls)
  python processor.py --input dataset.csv --analyze-only

  # High-throughput processing
  python processor.py --input large_dataset.parquet --concurrent 15 --rate-limit 100

Service Selection:
  • --service auto: Auto-detect best available service (default)
  • --service azure: Force Azure Content Safety/Prompt Shield
  • --service openai: Force OpenAI Moderation API

Dependencies:
  • Required: pandas, aiohttp, scikit-learn, numpy
  • Optional: pyarrow (Parquet), matplotlib (plots), openai (OpenAI API)
  • Install all: pip install pandas aiohttp scikit-learn numpy pyarrow matplotlib openai
...
Environment Variables:
  ...
  • OPENAI_API_KEY - OpenAI API key
    ...
    parser.add_argument('--input', '-i', type=str, required=True,
                       help='Input file path (supports .csv, .parquet, .pq)')
    
    # Authentication
    parser.add_argument('--endpoint', '-e', type=str,
                       help='Azure Content Safety endpoint URL')
    parser.add_argument('--api-key', '-k', type=str,
                       help='API key (Azure or OpenAI)')
    parser.add_argument('--service', '-s', choices=['auto', 'azure', 'openai'], default='auto',
                       help='Content safety service to use (default: auto)')
    ...
    parser.add_argument('--severity-threshold', type=int, default=2,
                       help='Severity threshold for blocking (0-6, default: 2, Azure only)')
    parser.add_argument('--timeout', type=int, default=30,
                       help='Request timeout in seconds (default: 30)')
    
    # Output options
    parser.add_argument('--output-dir', '-o', type=str, default='content_safety_results',
    ...
    parser.add_argument('--check-dependencies', action='store_true',
                       help='Check all dependencies and their versions')
...
async def main():
    ...
    try:
        ...
        logger.info("Multi-Cloud Content Safety Processor v5.1 initialized")
    ...
    try:
        ...
        file_extension = file_path.suffix.lower()
        if file_extension not in ['.csv', '.parquet', '.pq']:
            logger.error(f"Unsupported file format: {file_extension}")
            logger.error("Supported formats: .csv, .parquet, .pq")
            sys.exit(1)
        
        if file_extension in ['.parquet', '.pq'] and not PYARROW_AVAILABLE:
            logger.error("PyArrow is required for Parquet files. Install with: pip install pyarrow")
            sys.exit(1)
        ...
        # Save comprehensive results based on output format preference
        input_filename = Path(args.input).name
        
        if args.output_format == 'csv':
            # Force CSV-only output
            original_pyarrow = globals()['PYARROW_AVAILABLE']
            globals()['PYARROW_AVAILABLE'] = False
            try:
                ...
                globals()['PYARROW_AVAILABLE'] = original_pyarrow
        elif args.output_format == 'parquet':
            # Parquet-only output (if available)
            if not PYARROW_AVAILABLE:
                logger.error("PyArrow required for Parquet output. Install with: pip install pyarrow")
        ...
        if args.generate_plots and PLOTTING_AVAILABLE:
            logger.info("Plot generation would be implemented here")
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(1)        prompt_shield_payload = {
        ...
        try:
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                for endpoint_url, api_version, endpoint_type in test_endpoints:
                    try:
                        logger.info(f"Testing {endpoint_type.upper()} endpoint: {endpoint_url}")
                        
                        # Choose appropriate payload
                        if endpoint_type == 'prompt_shield':
                            current_payload = prompt_shield_payload
                        else:
                            current_payload = content_safety_payload
                        
                        async with session.post(endpoint_url, headers=headers, json=current_payload) as response:
                            if response.status == 200:
                                result = await response.json()
                                logger.info(f"✅ Working {endpoint_type.upper()} endpoint found: {endpoint_url}")
                                ...
                                # Log endpoint-specific capabilities
                                if endpoint_type == 'prompt_shield':
                                    logger.info("🛡️  Prompt Shield: Optimized for jailbreak and prompt injection detection")
                                else:
                                    logger.info("🔍 Content Safety: Multi-category content analysis")
                                
                                return True
                            else:
                                error_text = await response.text()
                                logger.warning(f"❌ {endpoint_type.upper()} endpoint failed {response.status}: {error_text[:200]}")
                                
                    except Exception as e:
                        logger.warning(f"❌ {endpoint_type.upper()} endpoint error: {str(e)}")
                        continue
                        
        except Exception as e:
            logger.error(f"❌ Connection error: {str(e)}")
        
        logger.error("❌ No working Azure endpoint found!")
        return False

    async def _call_openai_moderation(self, prompt: str) -> Dict[str, Any]:
        """Call OpenAI Moderation API"""
        if not OPENAI_AVAILABLE:
            return {'success': False, 'error': 'OpenAI library not available', 'latency_ms': 0}
    ...
    async def _call_azure_api(self, session: aiohttp.ClientSession, prompt: str) -> Dict[str, Any]:
        """Call Azure Content Safety or Prompt Shield API"""
        ...
        headers = {
            ...
            'User-Agent': 'MultiCloud-ContentSafety-Processor/5.1'
        }
        
        # Prepare payload based on endpoint type
        if self.endpoint_type == 'prompt_shield' or 'shieldPrompt' in self.working_endpoint:
            payload = {
                'userPrompt': prompt,
                'documents': []
            }
    ...
    def _parse_openai_response(self, response_data: Dict[str, Any]) -> Tuple[str, float, str]:
        ...
        try:
            ...
            if not results:
                return "ALLOWED", 0.0, "OpenAI: No results"
            ...
            # Calculate confidence based on max category score
            ...
            if flagged_categories:
                details = f"OpenAI: Flagged={flagged} | Categories: {','.join(flagged_categories)}"
            else:
                details = f"OpenAI: Flagged={flagged}"
            
            return decision, confidence_score, details
            
        except Exception as e:
            return "ERROR", 0.0, f"OpenAI parse error: {str(e)}"

    def _parse_azure_response(self, response_data: Dict[str, Any]) -> Tuple[str, float, str]:
        """Parse Azure Content Safety or Prompt Shield response"""
        try:
            # Prompt Shield response format
            if 'attackDetected' in response_data:
                ...
                # Extract confidence from Prompt Shield response
                confidence_score = 0.0
                ...
                # If no confidence in analyzeResults, use binary confidence
                if confidence_score == 0.0:
                    confidence_score = 0.9 if attack_detected else 0.1
                
                details = f"Azure Prompt Shield: Attack={attack_detected}"
                if 'analyzeResults' in response_data and response_data['analyzeResults']:
                    analysis_types = [result.get('category', 'Unknown') for result in response_data['analyzeResults']]
                    if analysis_types:
                        details += f" | Categories: {','.join(set(analysis_types))}"
                
                return decision, confidence_score, details
            
            # Content Safety response format
                ...
                if not categories_analysis:
                    return "ALLOWED", 0.0, "Azure Content Safety: No categories analyzed"
                ...
                severity_summary = "Azure Content Safety: " + "|".join(severity_details)
                ...
                return "ALLOWED", 0.0, "Unknown Azure response format"
                
        except Exception as e:
            return "ERROR", 0.0, f"Azure parse error: {str(e)}"

    def load_and_analyze_dataset(self, file_path: str) -> List[Dict[str, Any]]:
        """Enhanced dataset loading with comprehensive analysis for CSV and Parquet files"""
        ...
        try:
            ...
            if file_extension == '.csv':
                ...
                df = self._load_parquet_file(file_path)
            else:
                raise ValueError(f"Unsupported file format: {file_extension}. Supported formats: .csv, .parquet, .pq")
    ...
    def _load_csv_file(self, file_path: Path) -> pd.DataFrame:
                ...
                for sep in separators:
                    try:
                        ...
                        if len(df.columns) > 1:  # Valid if more than 1 column
                            logger.info(f"Loaded CSV with {encoding} encoding and '{sep}' separator")
                            return df
                    except:
                        continue
                        
                # If separators fail, try auto-detection
                df = pd.read_csv(file_path, encoding=encoding, sep=None, engine='python')
                logger.info(f"Loaded CSV with {encoding} encoding (auto-detected separator)")
                return df
        ...
        if df is None:
            raise ValueError("Could not read CSV with any encoding or separator combination")
    ...
    def _load_parquet_file(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            # Try loading with pandas (uses pyarrow backend)
            df = pd.read_parquet(file_path, engine='pyarrow')
            logger.info(f"Loaded Parquet file using pandas with pyarrow engine")
            
            # Log parquet file info
            try:
                parquet_file = pq.ParquetFile(file_path)
                logger.info(f"Parquet schema: {len(parquet_file.schema)} columns")
                logger.info(f"Parquet row groups: {parquet_file.num_row_groups}")
                    
            except Exception as e:
                logger.debug(f"Could not read detailed parquet info: {str(e)}")
            ...
            try:
                ...
                df = table.to_pandas()
                logger.info(f"Loaded Parquet file using pyarrow directly")
                return df
            except Exception as e2:
                raise ValueError(f"Could not load Parquet file with any method. Original error: {str(e)}, Alternative error: {str(e2)}")
    ...
    async def _process_single_prompt(self, session: aiohttp.ClientSession, prompt_data: Dict[str, Any]):
        """Enhanced single prompt processing for multiple services"""
        async with self.semaphore:
            ...
            # Call appropriate API based on detected service
            if self.detected_service == "openai":
                api_result = await self._call_openai_moderation(prompt)
            else:
                api_result = await self._call_azure_api(session, prompt)
            
            if api_result['success']:
                try:
                    decision, confidence_score, severity_scores = self._parse_responses(
                ...
                return EnhancedPrompt#!/usr/bin/env python3
"""
Multi-Cloud Content Safety Batch Processor - Production Ready v5.1
...
try:
    ...
    print("INFO: pyarrow not available. Install with: pip install pyarrow (for Parquet support)")
...
try:
    ...
    print("INFO: openai not available. Install with: pip install openai (for OpenAI Moderation support)")

# Configure logging
results_dir = Path("content_safety_results")
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(results_dir / 'content_safety_processor.log'),
...
class AdvancedMetricsCalculator:
    ...
    def calculate_comprehensive_metrics(results: List[EnhancedPromptResult]) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            # Core Metrics (Binary only - no macro averaging)
...
class MultiCloudContentSafetyProcessor:
    ...
    def __init__(self, 
                 ...
                 api_key: str = None,
                 service_type: str = "auto",  # auto, azure, openai
    ...
    async def discover_working_endpoint(self) -> bool:
        """Discover and validate working endpoint"""
        
        # OpenAI Moderation API
        if self.service_type == "openai" or (self.service_type == "auto" and not self.endpoint_url):
            return await self._test_openai_endpoint()
        
        # Azure endpoints
        if self.endpoint_url:
            return await self._test_azure_endpoints()
        
        logger.error("No endpoint configuration provided")
        return False

    async def _test_openai_endpoint(self) -> bool:
        """Test OpenAI Moderation endpoint"""
        if not OPENAI_AVAILABLE:
        ...
        try:
            ...
            logger.info("Testing OpenAI Moderation endpoint...")
            
            # Test with a simple message
            response = await self._call_openai_moderation("Hello, this is a test message")
            
            if response['success']:
                logger.info("✅ OpenAI Moderation endpoint working")
                ...
                logger.error(f"❌ OpenAI test failed: {response['error']}")
                return False
                
        except Exception as e:
            logger.error(f"❌ OpenAI endpoint test error: {str(e)}")
            return False

    async def _test_azure_endpoints(self) -> bool:
        """Test Azure Content Safety and Prompt Shield endpoints"""
        test_endpoints = [
            # Prompt Shield endpoints (prioritized for jailbreak detection)
            ...
            (f"{self.endpoint_url}/contentsafety/text:analyze?api-version=2023-10-01", "2023-10-01", "content_safety"),
        ]
        ...
        content_safety_payload = {
            'text': 'Hello world test message for endpoint discovery',
            'categories': ['Hate', 'SelfHarm', 'Sexual', 'Violence'],
            'outputType': 'FourSeverityLevels'
        }
        
        prompt_shield_payload = {
            'userPrompt': 'Hello world test message for endpoint